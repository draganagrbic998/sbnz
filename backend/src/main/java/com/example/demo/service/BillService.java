...
public class BillService {
	...
	public BillResponse create(BillRequest request) {
		...
		if (response.isValid()) {
			...
			Bill bill = new Bill(
				account, request.getType(), request.getMonths(), request.getBase(), 
				response.getEks() * response.getNks() / 100.0, request.getBase() + Utils.convertToCurrency(request.getType(), response.getReward())
			);
